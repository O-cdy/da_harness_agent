...
> 版本 v0.9 | 建立：2026-09-22 | 最后更新：2026-09-24 | 状态：**生效**（R-01 ~ R-42 + **R-49～R-105**）。ADR-029 的 R-43~R-48 **编号占用且不复用，不生效**（ADR-030）。ADR-068 起规则按 scope 组合加载；ADR-069 冻结动态平台与发布契约；ADR-070 校正 S0 实现表达；ADR-071 冻结公开仓库与数据安全边界。
...
| R-103 | **凭据零入库与泄露响应**（ADR-071）。凭据只经 env/Secret Store 注入；禁止进入 Git、CLI 参数、日志、trace、报告和 CI artifact。提交与 PR 必须 secret scan；命中真实凭据先撤销/轮换和审计，再清理历史 | 阻断 |
| R-104 | **数据库三层防写**（ADR-071）。生产数据访问必须同时具备独立只读账号、驱动只读会话和 SQL AST 只读校验；禁止 DDL/DML、事务写入、临时表、存储过程、文件读写、多语句、动态 `USE` 与越权对象 | 阻断 |
| R-105 | **数据最小化与出境默认拒绝**（ADR-071）。原始业务行与 PII 默认不落盘、不进报告、不传 LLM/MCP/外部服务；Artifact 按分级和 capability allowlist 流转。无明确授权必须 NoOp，禁止静默外传或换通路 | 阻断 |
