...
> 结构：**A canonical** → **B platform-native 官方口径** → **C 平台映射** → **D 已关闭口径清单**
> 版本 v0.6 | 建立：2026-09-22 | 最后更新：2026-09-24 | 状态：**部分生效** —— 已确认、目标平台核心 capability 完整且组件已认证的条目可作硬约束；动态平台与发布门禁见 ADR-069；S0 实现表达校正见 ADR-070。
...
| G-02 | 汇率 | **用户提供的月度固定汇率**，来源二选一：① Profile 登记的汇率表（优先）；② 用户提供的同构文件。**N+1 规则**：M 月报告用 M+1 月汇率；若 M+1 月未结束、库中无该月汇率，回退 M 月当月汇率。所用月份与数值入证据包；报告期有效金额仍无法折算时阻断跨币种 formal | 已确认（ADR-011 + ADR-069） |
| G-03 | 收入确认时点 | **按 canonical 下单事件**；退款/取消在**发生月**处理，不回溯调整原月。Shopify adapter 映射 `sale_date`，TikTok adapter 映射 `created_time`；平台官方支付/发货时点指标留在 platform-native | 已确认（ADR-010 + ADR-068/069） |
| G-04 | 时区 | **首期统一 UTC**。canonical 日/月切分只认 `order_created_date`；物理字段由 adapter 映射。禁止跨平台 SQL 引用 `sale_date` 或按站点本地时区重切 | 已确认（ADR-055 + ADR-068/069） |
...
| G-12 | 月报版本与发布等级 | `data_completeness(partial/final)` 与 `report_tier(dry_run/formal_partial/formal_final)` 正交。水位未齐但其它正式条件满足可出 formal_partial；核心源全部越过截止点+延迟窗口方可 formal_final。draft、未认证组件或未关闭 alignment 只可 dry-run。新版本不覆盖旧版 | 已确认（ADR-013 + ADR-069） |
...
| G-22 | 经营账户与市场层级 | canonical 原子 = `platform + account_id`；Profile 可映射 reporting site / market / region。账户永久保留，禁止进入 canonical 时直接合并成市场。当前 Profile 的“欧盟”是泛欧经营区 EU/DE/FR/IT/ES/IE/UA，UK 不进；禁止市场与成员账户混加 | 已确认（ADR-060/061 + ADR-069） |
| G-23 | 口径可追溯 | 每条「已确认」G/M/R 必须能追到 ADR + 本文件条目。profile 参数必须有 `source`。报告数字必须能追到指标编码 + SQL/代码 + 快照 hash。状态「已定」不得进报告 | 已确认（ADR-062） |
| G-24 | 平台映射与能力 | canonical 指标只消费 adapter 输出。每次 Run 显式选择 `target_platforms`；核心能力缺失不可 waiver，可选能力缺失须先逐项询问，确认本次跳过后记录 run-only waiver。任何缺口不得以 0 代替 | 已确认（ADR-068 + ADR-069） |
| G-25 | 多平台完整度 | 每个目标平台/核心源独立记录截止点、延迟窗口、水位、hash 与 completeness。水位未齐可 formal_partial；全部目标核心源齐备后出新 formal_final；禁止上期数据补齐或覆盖旧版 | 已确认（ADR-068 + ADR-069） |
...
| M102 | 净销售额 Net Revenue | 商业商品成交净额，退款按事件发生月冲减 | `SUM(item_gross_amount + seller_discount_amount) + SUM(refund_net_effect)`；adapter 将退款规范为对净额的 signed effect，并以 `refund_kind` 区分 line refund / order adjustment。取消、样品、非商业赠送剔除；税与运费不进入 | 本位币 | 已确认（ADR-010/019/022/023/050/052/053 + ADR-068/069） |
| M103 | 订单数 Orders | 有效商业订单数；取消与样品单剔除，退款单保留原下单月 | `COUNT(DISTINCT platform + account_id + order_id)`；平台原始订单键由 adapter 映射 | 单 | 已确认（ADR-010 + ADR-049 + ADR-068/069） |
...
| M104 | 件数 Units | 商业售出件数，按下单时点；取消/样品/赠品剔除。退款单件数保留在原下单月，退货件数只用于 M207b，不冲减 M104 | canonical `SUM(commercial_quantity)`；当前 profile 的 NSSKU/型号展开仍遵守 G-17 与两套件数分标注。平台过滤见 C 节 | 件 | 已确认（ADR-010 + ADR-045 + ADR-052/053 + ADR-068；旧文案由 ADR-070 校正） |
...
> **适用范围硬限制**：仅适用于 `customer_identity` capability 已认证的平台/账户。缺身份能力的平台不得补 0、不得塞入“未识别”凑跨平台客户层闭合；按可选能力流程询问并省略。

| 编码 | 指标 | 定义 | 公式 | 状态 |
|---|---|---|---|---|
| M501 | 下单顾客数 Customers | 期内下过单的去重已识别顾客数；身份默认只在 `platform + account_id` 命名空间内有效 | `COUNT(DISTINCT customer_subject_hash)`；Shopify 映射 gid，TikTok 首阶段 capability=unsupported | 已确认（ADR-017 + ADR-062 + ADR-068/069） |
| M502 | 新客数 New Customers | 期内**首次**下单的已识别顾客 | 该 `customer_subject_hash` 的 `MIN(order_created_date)` 落在报告期。Shopify 禁用注册时间 `customer_created_at`；TikTok 首阶段 identity capability=unsupported | 已确认（ADR-017 + R-39 + ADR-062 + ADR-068/069） |
| M503 | 老客数 Returning Customers | 期内下单且此前已有历史订单的已识别顾客 | 同一 `platform + account_id` 命名空间内首单日 < 报告期且期内有下单。Shopify 实测闭合；TikTok 首阶段不计算 | 已确认（ADR-017 + R-39 + ADR-062 + ADR-068/069） |
...
**主键规则（ADR-062 / ADR-068 / ADR-069）**：canonical 只存“平台 + 账户命名空间 + 不可逆 subject hash”。Shopify adapter 输入为 `shopify_customer_gid`；跨平台/账户合并只能用当前组织已确认的 identity map，禁止用 email/手机号自动合并。
**统计范围（R-40）**：默认在 `platform + account_id` 内统计。跨账户指标必须声明 identity map 版本，不得与账户内口径混用。
...
| 编码 | 指标 | 公式 | 状态 |
|---|---|---|---|
| M301 | 目标达成率 | 实际值(CNY 元) / (目标值 × 10000)；分子分母均为 M102。只在 `targets` capability 可用且目标映射到相同账户/汇总范围时计算；缺目标先询问，不得补 0或生成全渠道达成率 | 已确认（ADR-016/020/028 + ADR-068/069） |
| M302 | 同比 YoY | (本期 − 去年同期) / 去年同期。按 `platform + account_id` 的 `MIN(order_created_date)` 判断完整月数 ≥12；不够龄不出。合计同比只含够龄账户；新账户当期进规模、不进同比分子分母。禁止分母填 0或写死名单 | 已确认（ADR-059 + ADR-069） |
| M303 | 环比 MoM | (本期 − 上期) / 上期。新账户可出；上期为空则不出，禁止填 0。停运账户按 platform pack 排除 | 已确认（ADR-059 + ADR-069） |

### A4 异动类

| 编码 | 指标 | 公式 | 状态 |
|---|---|---|---|
| M401 | 量价拆解 | 解释 ΔM102。量 = canonical 商业件数；价 = 同范围 ASP（M102/M104）。量贡献 = (本期件数 − 上期件数) × 上期 ASP；价贡献 = (本期 ASP − 上期 ASP) × 本期件数；残差单列并闭合。当前 Shopify 商品展开仍禁止用 G-17 NS/型号件数替代商业件数 | 已确认（ADR-063 + ADR-069） |
| M402 | 维度归因贡献度 | 解释 ΔM102。每次一层、层内互斥可加。账户层依赖核心订单；商品层需 product_identity；客户层需 customer_identity。只对 capability 完整的目标范围执行；禁止把缺能力平台塞入未识别桶。市场不得与成员账户混加 | 已确认（ADR-064 + ADR-069） |
...
| M102 净销售额 | `gross_sales + discounts`；lineitems=`line_refund`，adjustments=`order_adjustment`，均规范成 signed effect | `sku_subtotal_after_discount`；returns=`line_refund`，按 `event_date` 冲减；取消退款不二次冲 | OPS − 退款报告 | 商品净额不含运费/税；退款按事件月且 kind 不得丢失 |
| M103 订单数 | `order_name` | `order_id`；取消/样品剔除 | 需订单报告 | canonical 主键=`platform + account_id + order_id`，禁止跨账户碰撞 |
...
| ~~D-10~~ | ~~客户唯一标识口径~~ **已被 ADR-062/068/069 修订**：Shopify 原始主键为 gid；canonical 为 `platform + account_id` 命名空间内 subject hash；游客单单列未识别 | M501–M507 |
...
| ~~D-23~~ | ~~M103 原始订单键~~ **ADR-069 修订**：Shopify adapter=`order_name`；TikTok adapter=`order_id`；canonical=`platform + account_id + order_id` | M103 |
...
| ~~D-29~~ | ~~G-04 时区 / G-05 财月~~ **ADR-069 修订**：UTC + 自然月 × canonical `order_created_date`；物理列由 adapter 映射 | G-04 / G-05 |
| ~~D-30~~ | ~~双库是否拆两套 env~~ **已确认 2026-09-24**：同一连接；schema 走 profile；不拆 `MYSQL_DATABASE_*`（ADR-056） | 连接 |
| ~~D-31~~ | ~~C1/C2 同模板是否再审~~ **ADR-069/070 收窄**：只有 C1/C2 各自完整指纹一致才可复用；平台集合变化重审 C1；C3 每个候选包每次审核 | 卡点 |
...
| ~~D-36~~ | ~~新老客主键~~ **ADR-069 修订**：Shopify adapter 输入 gid；canonical 使用平台/账户命名空间内 subject hash；空值未识别，跨账户只用已确认 identity map | M501–M507 |
...
| ~~D-39~~ | ~~M402 贡献度如何拆~~ **ADR-069 补充 capability 门禁**：账户层属核心；商品/客户层仅对相应 capability 完整范围执行；禁止把缺能力平台塞入未识别桶 | M402 |
